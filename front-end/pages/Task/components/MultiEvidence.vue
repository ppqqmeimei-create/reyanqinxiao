<template>
	<view class="multi-evidence">
			...
			<camera class="camera-view" :device-position="cameraPosition" flash="off" @error="handleCameraError">
				<cover-view class="sensor-overlay">
					<cover-view class="sensor-item">
						<cover-image class="sensor-icon" src="/static/icons/smell.png"></cover-image>
					...
					<cover-view class="sensor-item">
						<cover-image class="sensor-icon" src="/static/icons/fiber.png"></cover-image>
						<cover-view class="sensor-data">
					...
					<cover-view class="sensor-item">
						<cover-image class="sensor-icon" src="/static/icons/infrared.png"></cover-image>
						<cover-view class="sensor-data">
		...
		<view class="evidence-controls">
			<view class="control-btn control-btn-record" @tap="handleToggleRecording" :class="{ 'recording-active': isRecording }">
				<image class="btn-icon" src="/static/icons/task-evidence-record.png" mode="aspectFit"></image>
			</view>
			<view class="control-btn capture-btn" @tap="handleCapture">
				<image class="btn-icon capture-icon" src="/static/icons/task-evidence-capture.png" mode="aspectFit"></image>
			</view>
			<view class="control-btn control-btn-switch" :class="{ 'switch-active': isSwitching }" @tap="handleSwitchCamera">
				<image class="btn-icon" src="/static/icons/task-evidence-switch-camera.png" mode="aspectFit"></image>
			</view>
			<view class="control-btn quick-capture-btn" @tap="handleQuickCapture">
				<view class="btn-glow"></view>
				<image class="btn-icon quick-capture-icon" src="/static/icons/task-evidence-quick-capture.png" mode="aspectFit"></image>
...
function handleQuickCapture() {
	uni.vibrateLong()
	uni.showToast({ title: '一键取证已启动', icon: 'success' })
}
...
.control-btn-record { background: rgba(255,77,79,0.16); }
.control-btn-switch { background: rgba(0,212,255,0.14); }
.capture-btn { width: 120rpx; height: 120rpx; background: rgba(255,255,255,0.94); border-color: rgba(255,255,255,0.92); }
...
.capture-icon { width: 56rpx; height: 56rpx; }
.quick-capture-icon { width: 54rpx; height: 54rpx; }
